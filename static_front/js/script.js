...
$(document).ready(function(){
	...
	*******************************************************************************************/
	...
	var colors = ['rgba(255,255,255,0.4)', 'rgba(202,249,126,0.4)', 'rgba(247,188,106,0.4)', 'rgba(138,190,149,0.4)', 'rgba(250,176,206,0.4)', 'rgba(136,243,242,0.4)', 'rgba(249,245,154,0.4)'];
	var fonts = ['Special Elite', 'Gloria Hallelujah', 'Coming Soon', 'Satisfy', 'Delius Swash Caps', 'Leckerli One', 'Shadows Into Light Two'];
	...
	******************************************************************************************/
	...
	var hideAndShow = function(target, activeTargets, eventNumber, elements, elementsData, ourHeightsSection){
		var selfHeightIndex = (eventNumber-1);
		for(var i = 0; i < elements.length; i++ ){
			if( $(elements[i]).attr(elementsData) != eventNumber ){
				TweenLite.to(elements[i], 0.6, {css:{height:0, paddingTop: 0, paddingBottom:0}, ease:Power4.easeOut}); 
				...
				if($(elements[i]).attr('data-shown') == "true"){
					...
					TweenLite.to(elements[i], 0.6, {
						css:{height:selfHeight, paddingTop: '10px', paddingBottom:'10px'}, 
						ease:Power4.easeIn, 
	...
	*****************************************************************************************/
	$(document).on('click', '.icon-menu-link', function(e){
		e.preventDefault();
		hideAndShow($(this), $('.desc-story-item'), $(this).parent().attr("data-event"), $('.story-event'), 'data-event-body', ourHeights.story);
	});
	$(document).on('click', '.desc-story-item', function(e){
		e.preventDefault();
		hideAndShow($(this), $('.desc-story-item'), $(this).attr("data-event"), $('.story-event'), 'data-event-body', ourHeights.story);
	});

	// Info menu icons
	$(document).on('click', '.info-menu-item', function(e){
		e.preventDefault();
		hideAndShow($(this), $('.desc-info-item'), $(this).attr("data-info"), $('.info-article'), 'data-info-body', ourHeights.info);
	});
	...
	$(document).on('click', '.talk-menu-item', function(e){
		e.preventDefault();
		hideAndShow($(this), $('.desc-talk-item'), $(this).attr('data-post'), $('.post'), 'data-post-body', ourHeights.talk);
	});
	...
	*******************************************************************************/
	...
	var animatingMenuLogic = function(menu, menuItems, innerButton){
		...
		landscapeMax = fullPageWidth < fullPageHeight ? fullPageHeight : fullPageWidth;
...
if (window.innerWidth > 1024){
	...
	$(document).on('click',function(e){
		if (e.target != menuButton.get(0)) {
			TweenLite.to(mobileMenu, 0.3, {css:{right:-landscapeMax}, ease:Power4.easeOut});
		} else{
			return;
		}
	});
	...
	*****************************************************************************/
	...
	$('#bowl-submit').on('click', document, function(e){
		...
		$.getJSON('/bowl', bowl_message, function(messageReceived){
			...
			$('#bowl-container').append(				
				'<article class="bowl-message" id="message-' + messageReceived.guest_id + '" data-bowl-user="' + messageReceived.guest_id + '" style="font-family:'+ tempFont +'; background-color:'+ tempColor+'">' +
					'<img class="bowl-logo" src="/static_front/img/general/logo-bowl.png" alt="Pedro & Lucía se casan">' +
					'<p class="bowl-body">'+ messageReceived.bowl_message + '</p>' +
					'<span class="bowl-guest">' + messageReceived.nickname + '</span>' +
				'</article>'
			);
	...
	for (var i = 0; i < messages.length; i++){
		...
		var tempFont = fonts[Math.floor(Math.random()*fonts.length)];
		var tempColor = colors[Math.floor(Math.random()*colors.length)];
		$($(messages)[i]).css({'background-color':tempColor, 'font-family': tempFont});
	}
	...
	*******************************************************************************************/
	$('.comment-action').on('click', function(e){
		...
		$('#comment-submit').one('click', function(e){
			e.preventDefault;
			comment.body = $(this).prev().val();
			comment.title = $(this).prev().prev().val();
			...
			$.getJSON('/comment', comment, function(receivedComment){
				...
				$(postBody).prepend(
					'<div class="comment" id="comment-'+ receivedComment.comment_id +'">' +
						'<div class="guest-img '+ receivedComment.color+'"></div>' +
						'<div class="comment-guest">' +
							
							'<h4 class="comment-nickname">' + receivedComment.guest + '</h4>' +
							'<span class="comment-time">' + receivedComment.timestamp + '</span>' +
						'</div>' +
						'<div class="comment-body">' +
							'<h4>'+ receivedComment.title +'<h4>'+
							'<p>' + receivedComment.body  +'</p>' +
							'<div class="reply-btns">' +
								'<div class="reply-action" data-comment-id="'+receivedComment.comment_id+'">responder</div>'+	
							'</div>' +	
	...
	*******************************************************************************************/
	$('.ver-mas-talk').on('click', function(){
		var attr = $(this).closest('.post').attr('data-post-body');
		...
		$.getJSON('/getcomments', data, function(receivedComments){
			...
			for( var key in receivedComments){
				if( key != 'status'){
					...
					var buttons = '<div class="reply-btns"><div class="reply-action" data-comment-id="'+ comment.id + '">responder</div>';
					if (repliesExist){
					...
					for(var key in comment.replies){
						...
						tempReplies += 
									'<div class="reply">'+
										'<div class="guest-img '+ reply.color +'"></div>'+
										'<div class="comment-guest">'+
											
											'<h4 class="comment-nickname">'+reply.guest +'</h4>'+
					...
					'<div class="comment" id="comment-' + comment.id +'">'+
								'<div class="guest-img ' + comment.color +'"></div>'+
								'<div class="comment-guest">'+
									'<h4 class="comment-nickname">'+comment.guest+'</h4>'+
	...
	*******************************************************************************************/
	...
	$(document).on('click', '.reply-read', function(e){
		...
		$(this).parents('.comment-body').find('.replies-container').slideToggle();
		...
		$('#reply-submit').one('click', function(e){
			...
			$.getJSON('/reply', reply, function(receivedReply){
				...
				if( $(comment).find('.reply-read').length < 1  ){
					...
					$(comment).find('.reply-btns').append('<div class="reply-read" >ver respuestas</div>');
				}
				

				// append the new reply
				$(comment).find('.replies-container').append(
					'<div class="reply" style="display: block">' +
						'<div class="guest-img '+ receivedReply.color +'"></div>' +
						'<div class="comment-guest">' +
							
							'<h3 class="comment-nickname">' + receivedReply.guest + '</h3>' +
							'<span class="comment-time">' + receivedReply.timestamp + '</span>' +
						'</div>' +
						'<div class="reply-body">'+
							'<p>' + receivedReply.body + '</p>' +
						'</div>' +
					'</div>'
				).toggle();
	...
	*******************************************************************************************/
	...
	$('#clock').countdown({
		until: wedding,
		compact: true,
		layout: '<span class="countdown-text">Faltan: </span><b class="countdown-text">{dn} d {hnn} h {mnn} m {snn} s</b>',
...
xhr.open('GET', 'http://coffeemaker.herokuapp.com/instagram.json?q=pedroylucia', true);
xhr.send();
...
$(document).on('mouseenter', '.story-menu-link', function(e) {
	var index = $(e.target).parent().attr('data-event');
...
$(document).on('mouseenter', '.info-menu-link', function(e) {
	var index = $(e.target).parent().attr('data-info');
...
$(document).on('mouseenter', '.talk-menu-link', function(e) {
	var index = $(e.target).parent().attr('data-post');
